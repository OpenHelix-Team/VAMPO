...
def train(cfg: DictConfig) -> None:
    ...
    if accelerator.is_main_process:
        ...
        experiment_dir = f"output/policy/{uuid}"
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for test_batch in test_loader:
            with torch.no_grad():
                ...
                val_losses.append(val_loss["validation_loss"])

        val_losses = torch.tensor(val_losses, device=device)
        gathered_val_losses = accelerator.gather(val_losses)
        
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            if gathered_val_losses.numel() > 0:
                total_val_loss = torch.mean(gathered_val_losses).item()
            else:
                total_val_loss = 0.0
