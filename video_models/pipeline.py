...
from grpo.eas_with_logprob import eas_step_with_logprob, stochastic_eds_step_with_logprob
from grpo.eds_with_logprob import eds_step_with_logprob
...
class EDSStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    def __call__(
        self,
        image,
        text,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1
                # latents = self.scheduler.step(noise_pred, t, latents).prev_sample
                latents, _ = eas_step_with_logprob(
                    self.scheduler,  
                    noise_pred, 
                    t, 
                    latents, 
...
class SamplingStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        image,
        ...
        batch_size = image_embeddings.shape[0]
        image_embeddings = image_embeddings.repeat(num_videos_per_prompt, 1, 1)
        if do_classifier_free_guidance:
            negative_image_embeddings = torch.zeros_like(image_embeddings)
            image_embeddings = torch.cat([negative_image_embeddings, image_embeddings])
        ...
        # Repeat the image latents for each frame so we can concatenate them with the noise
        # image_latents [batch, channels, height, width] ->[batch, num_frames, channels, height, width]
        image_latents = image_latents.unsqueeze(1).repeat(num_videos_per_prompt, num_frames, 1, 1, 1)
        # mask = repeat(mask, '1 h w -> 2 f 1 h w', f=num_frames)
        # 5. Get Added Time IDs
        added_time_ids = self._get_add_time_ids(
            fps,
        ...
        base_latents = self.prepare_latents(
            batch_size, # <-- 关键修改：只生成 batch_size 份
            num_frames,
            num_channels_latents,
            height,
            width,
        ...
        latents = base_latents.repeat(num_videos_per_prompt, 1, 1, 1, 1)
        ...
        self._guidance_scale = guidance_scale
        all_latents_trajectory = []
        all_log_probs_trajectory = []
        all_sigma_ups_trajectory = []
        all_added_time_ids = []
        # 8. Denoising loop
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        self._num_timesteps = len(timesteps)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                if i == sde_start:
                    all_latents_trajectory.append(latents)
                if sde_start <= i < sde_end:
                    latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                    latent_model_input = torch.cat([latent_model_input, image_latents], dim=2)
                    ...
                    # all(latent_model_input[0:8]==latent_model_input[8:16])
                    # all(image_embeddings[0:8]==image_embeddings[8:16])
                    # all(added_time_ids[0:8]==added_time_ids[8:16])
                    # all(noise_pred[0:8]==noise_pred[8:16])
                    if do_classifier_free_guidance:
                        noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)
                        noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_cond - noise_pred_uncond)

                    latents, log_probs, sigma_ups = eas_step_with_logprob(
                        self.scheduler,
                        noise_pred,
                        t,
                        latents,
                    ...
                    # latents, log_probs, sigma_ups = stochastic_eds_step_with_logprob(
                    #     self.scheduler,
                    #     noise_pred,
                    #     t,
                    #     latents,
                    ...
                    all_latents_trajectory.append(latents)
                    all_log_probs_trajectory.append(log_probs)
                    all_sigma_ups_trajectory.append(sigma_ups)
                    
                    half_size = added_time_ids.shape[0] // 2
                    all_added_time_ids.append(added_time_ids[:half_size])
                    ...
                    with torch.no_grad():
                        ...
                        bsz, frames, c, h, w = latents.shape
                        latents_4d = latents.reshape(bsz * frames, c, h, w)
                        noise_pred_4d = noise_pred.reshape(bsz * frames, c, h, w)
                        
                        latents_4d = self.scheduler.step(
                            model_output=noise_pred_4d,
                            timestep=t,
                            sample=latents_4d,
                        ).prev_sample

                        latents = latents_4d.reshape(bsz, frames, c, h, w)
            # for i, t in enumerate(timesteps):
            #     # expand the latents if we are doing classifier free guidance
            #     latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            #     latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
        ...
        return latents, all_latents_trajectory, all_log_probs_trajectory, all_sigma_ups_trajectory, all_added_time_ids

class ActorStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    def __call__(
        self,
        image,
        text,
        ...
        # mask = repeat(mask, '1 h w -> 2 f 1 h w', f=num_frames)
        # 5. Get Added Time IDs
        added_time_ids = torch.cat([batch_added_time_ids] * 2) if do_classifier_free_guidance else batch_added_time_ids
        added_time_ids = added_time_ids.to(device)
        ...
        # _, log_probs, sigma_up = stochastic_eds_step_with_logprob(
        #     self.scheduler,  
        #     model_output=noise_pred, 
        #     timestep=t, 
        #     sample=latents, 
        #     generator=generator,
        #     prev_sample=batch_next_latents
        # )
        _, log_probs, sigma_up = eas_step_with_logprob(
            self.scheduler,  
            model_output=noise_pred, 
            timestep=t, 
            sample=latents, 
